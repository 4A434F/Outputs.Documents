...
public sealed class DocumentScannerTests
{
    ...
    [Fact]
    public void CreateDescriptor_UsesNoDocumentModelWhenDefaultModelParameterIsMissing()
    {
        var descriptor = DocumentScanner.CreateDescriptor(typeof(MissingModelParameterTemplate));

        Assert.NotNull(descriptor);
        Assert.Equal(typeof(NoDocumentModel), descriptor.ModelType);
        Assert.Equal(typeof(MissingModelParameterTemplate), descriptor.BodyTemplateType);
        Assert.Equal("Model", descriptor.ModelParameterName);
    }
