...
```bash
docker build \
  -f tools/Outputs.Documents.Domain.VectorStore.McpServer/Dockerfile \
...
docker mcp profile server add default \
  --server file://tools/Outputs.Documents.Domain.VectorStore.McpServer/docker-mcp-server.yaml
```
