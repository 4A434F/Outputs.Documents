...
public sealed class DomainVectorStore : IDomainVectorStore
{
    ...
    public IReadOnlyList<DomainEmbeddingDescriptor> Describe(Type domainType)
    {
        return DomainEmbeddingDescriptorReader.Describe(domainType);
    }
    ...
    {
        var records = CreateRecords(modelName, domainType, embeddings);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var recordList = registrations
            .SelectMany(registration => CreateRecords(modelName, registration.DomainType, registration.Embeddings))
            .ToList();
    ...
    private static IReadOnlyList<EmbeddingRecord> CreateRecords(
        string modelName,
        Type domainType,
        IEnumerable<DomainEmbeddingValue> embeddings)
    {
        ...
        var descriptors = DomainEmbeddingDescriptorReader.Describe(domainType);
        var descriptorIds = descriptors
            .Select(descriptor => descriptor.Id)
            .ToHashSet(StringComparer.Ordinal);
        var embeddingsById = embeddings.ToDictionary(
            embedding => embedding.DescriptorId,
            StringComparer.Ordinal);

        var missingIds = descriptorIds
            .Where(id => !embeddingsById.ContainsKey(id))
            .Order(StringComparer.Ordinal)
            .ToList();
        ...
        {
            throw new ArgumentException(
                $"Domain type '{domainType.FullName}' is missing embeddings for descriptors: {string.Join(", ", missingIds)}.",
                nameof(embeddings));
        }

        var unknownIds = embeddingsById.Keys
        ...
        {
            throw new ArgumentException(
                $"Domain type '{domainType.FullName}' received embeddings for unknown descriptors: {string.Join(", ", unknownIds)}.",
                nameof(embeddings));
        }
        ...
        return descriptors
            .Select(descriptor =>
            {
                var value = embeddingsById[descriptor.Id];
